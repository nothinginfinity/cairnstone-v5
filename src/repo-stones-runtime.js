...
export async function createRepoStonesFromBody(body, env, deps) {
  ...
  for (const file of candidates) {
    ...
    if (reuseUnchanged && prior?.sha && prior.sha === file.sha) {
      reused.push({
        path: file.path,
        stone_hash: prior.stone_hash,
        refs: prior.refs,
        bytes: prior.bytes || file.size,
        sha: file.sha,
        reused_from: prior.stone_hash
      });
      continue;
    }
    ...
    if (prior?.stone_hash && prior.stone_hash !== result.stone_hash) {
      updated.push({
        ...item,
        previous_stone_hash: prior.stone_hash,
        previous_sha: prior.sha || null
      });

      if (linkSupersedes) {
        const edge = await deps.linkStonesFromBody({
          from_hash: result.stone_hash,
          to_hash: prior.stone_hash,
          edge_type: "supersedes",
          note: `Repo file ${file.path} supersedes previous Git SHA ${prior.sha || "unknown"}`
        }, env);
